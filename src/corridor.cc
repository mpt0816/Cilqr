...
bool Corridor::BuildCorridorConstraints(
  ...
  env_->QueryStaticObstaclesPoints(&obstacle_points, config_.is_multiple_sample);
  ...
  for (const auto& pt : trajectory.trajectory()) {
    ...
    env_->QueryDynamicObstaclesPoints(pt.time, &obstacle_points, config_.is_multiple_sample);
...
bool Corridor::BuildCorridor(
  ...
  if (isOriginAVertex) {
    ...
    double dx = (filterd_points[vertexIndice[last_index]].x() + origin_x + filterd_points[vertexIndice[next_index]].x()) / 3 - origin_x;
    double dy = (filterd_points[vertexIndice[last_index]].y() + origin_y + filterd_points[vertexIndice[next_index]].y()) / 3 - origin_y;
